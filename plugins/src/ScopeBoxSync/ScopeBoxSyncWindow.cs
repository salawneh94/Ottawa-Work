...
public class ScopeBoxSyncWindow : BimFlowWindow
{
    private readonly ComboBox _scopeBoxBox = BimFlowUi.ComboBox();
    ...
    public ScopeBoxSyncWindow(List<Element> scopeBoxes, List<View> candidateViews) : base("BIMFlow — Scope Box Sync", minWidth: 420)
    {
        _scopeBoxes = scopeBoxes;

        var root = new StackPanel();
        root.Children.Add(BimFlowUi.TitleBar("📦", "Scope Box Sync", "Apply one scope box to a set of views in one pass."));

        var scopeStack = new StackPanel();
        scopeStack.Children.Add(BimFlowUi.FieldLabel("Scope box to apply"));
