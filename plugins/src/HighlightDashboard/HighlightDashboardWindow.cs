...
/// BIMFlow window that hands a decision to Command.cs. Command.cs then
...
/// </summary>
public class HighlightDashboardWindow : BimFlowWindow
{
    ...
    {
        new("🎨", "Color Code", "Color-code any category by any parameter value — pick a palette, preview live, then apply as real persistent view filters.", "BIMFlow.OverrideByParam.Command"),
        new("🧱", "HL Exterior", "Toggle a red color highlight on every exterior wall in the active view.", "BIMFlow.HighlightExterior.Command"),
        new("🧱", "HL Interior", "Toggle a blue color highlight on every interior wall in the active view.", "BIMFlow.HighlightInterior.Command"),
        new("☁️", "PC Color", "Tint point cloud instances with a preset or custom color, all in view or just the selection.", "BIMFlow.PointCloudColorizer.Command"),
        new("🌡️", "PC Heatmap", "Compare walls against point cloud scan data and color-code deviations: green/yellow/red by tolerance.", "BIMFlow.PointCloudHeatmap.Command"),
    };
    ...
    public HighlightDashboardWindow() : base("Ottawa Tools — Highlight", minWidth: 620)
    {
        ...
        root.Children.Add(BimFlowUi.TitleBar("🎯", "Highlight", "Every Select/Highlight tool in one place — pick one to run it."));

        var grid = new UniformGrid { Columns = 2 };
        foreach (var entry in Entries)
        {
            var card = BimFlowUi.CardButton(entry.Icon, entry.Title, entry.Description);
