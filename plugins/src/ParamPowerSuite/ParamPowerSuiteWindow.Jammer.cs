...
{
    private readonly TextBox _jamParamNameBox = BimFlowUi.TextBox();
    private readonly TextBox _jamFilePathBox = BimFlowUi.TextBox();
    private readonly TextBox _jamGroupNameBox = BimFlowUi.TextBox();
    private readonly ComboBox _jamDataTypeBox = BimFlowUi.ComboBox();
    private readonly ComboBox _jamGroupTypeBox = BimFlowUi.ComboBox();

    private UIElement BuildJammerTab()
    {
        var stack = new StackPanel();
        stack.Children.Add(BimFlowUi.SectionHeader("Jammer"));
        stack.Children.Add(BimFlowUi.FieldLabel("Replaces a LOCAL family parameter with a SHARED one of the same name, in place — preserves every instance's current value and any formula. Operates on the distinct loadable families among the loaded elements (system-family categories like Walls/Floors have no family document and are not applicable)."));

        stack.Children.Add(BimFlowUi.FieldLabel("Parameter name (must already exist as a local, non-shared family parameter)"));
        stack.Children.Add(_jamParamNameBox);

        stack.Children.Add(BimFlowUi.FieldLabel("Shared parameter file"));
        ...
        _jamGroupNameBox.Text = "BIMFlow";
        stack.Children.Add(_jamGroupNameBox);

        stack.Children.Add(BimFlowUi.FieldLabel("Data type (used only if this exact parameter doesn't already exist in the file)"));
        ...
        stack.Children.Add(BimFlowUi.FieldLabel("Parameter group / discipline"));
        ...
        var jamButton = BimFlowUi.DangerButton("Jam to shared parameter");
    ...
    private void RunJammer()
    {
        ...
        var groupName = string.IsNullOrWhiteSpace(_jamGroupNameBox.Text) ? "BIMFlow" : _jamGroupNameBox.Text;
