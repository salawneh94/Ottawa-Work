...
/// </summary>
public class RoomTaggerWindow : BimFlowWindow
{
    private readonly List<CheckBox> _checkBoxes = new();
    private readonly TextBox _numberParamBox = BimFlowUi.TextBox();
    private readonly TextBox _nameParamBox = BimFlowUi.TextBox();
    ...
    public RoomTaggerWindow() : base("BIMFlow — RoomTagger")
    {
        var root = new StackPanel();
        root.Children.Add(BimFlowUi.TitleBar(
            "🏷️",
            "Room Tagger",
            "Write each room's number/name onto every element found inside it — for categories Revit doesn't auto-tag with a Room field."));

        root.Children.Add(BimFlowUi.SectionHeader("Categories to tag"));
        ...
        root.Children.Add(BimFlowUi.SectionHeader("Target parameters"));
        var paramStack = new StackPanel { Margin = new Thickness(0, 0, 0, 20) };
        paramStack.Children.Add(BimFlowUi.FieldLabel("Room number parameter (must already exist on these categories — leave blank to skip)"));
        _numberParamBox.Text = "Room Number";
        paramStack.Children.Add(_numberParamBox);
        paramStack.Children.Add(BimFlowUi.FieldLabel("Room name parameter (must already exist on these categories — leave blank to skip)"));
        _nameParamBox.Text = "Room Name";
        paramStack.Children.Add(_nameParamBox);
        root.Children.Add(BimFlowUi.Card(paramStack));
        ...
        var runButton = BimFlowUi.PrimaryButton("Tag elements");
